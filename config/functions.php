...
// 获取介绍文本（支持API地址、直接文字或本站一言）
function getIntroText($intro) {
    // 检查是否启用本站一言
    try {
        $db = getDB();
        $config = $db->query("SELECT use_local_hitokoto FROM website_config LIMIT 1")->fetch();
    ...
    // 检测是否是URL（API地址）
    if (filter_var($intro, FILTER_VALIDATE_URL) || preg_match('/^https?:\/\//i', $intro)) {
        try {
            // 使用 cURL 获取 API 数据
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $intro);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_TIMEOUT, 5); // 5秒超时
            curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); // 如果是HTTPS
            ...
            if ($httpCode == 200 && $response) {
                ...
                // 如果是JSON格式，尝试获取常见的文本字段
                if (json_last_error() === JSON_ERROR_NONE && is_array($data)) {
                    // 尝试常见的字段名
                    $possibleKeys = ['text', 'sentence', 'content', 'message', 'data', 'hitokoto', 'quote', 'forward'];
