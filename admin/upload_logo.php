...
if (!in_array($extension, $allowedExtensions)) {
    http_response_code(400);
    echo json_encode(['error' => '只支持 PNG, JPG, GIF, WEBP 格式']);
    exit;
}
...
if ($file['size'] > 5 * 1024 * 1024) {
    http_response_code(400);
    echo json_encode(['error' => '文件大小不能超过 5MB']);
    exit;
}
...
// 删除旧的 logo 文件
foreach (['logo.png', 'logo.jpg', 'logo.jpeg', 'logo.gif', 'logo.webp'] as $oldFile) {
...
if (move_uploaded_file($file['tmp_name'], $uploadPath)) {
    ...
    try {
        $db = getDB();
        // 检查字段是否存在
        $columns = $db->query("SHOW COLUMNS FROM website_config LIKE 'logo'")->fetchAll();
        if (count($columns) > 0) {
            $stmt = $db->prepare("UPDATE website_config SET logo=? WHERE id=1");
            $stmt->execute([$url]);
        }
    } catch (Exception $e) {
        // 忽略数据库错误，文件已经上传成功
        error_log("Logo database update error: " . $e->getMessage());
    }
