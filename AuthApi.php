...
class BehaviorAuth {
    ...
    private function analyzeEnvironment($env) {
        // [DEBUG] 调试版:只记录违规项不拦截,便于在 Trae/Electron 等浏览器中通过验证
        $violations = [];
        if (!empty($env['webdriver']))      $violations[] = 'webdriver';
        if (!empty($env['auto_phantom']))   $violations[] = 'auto_phantom';
        if (!empty($env['auto_nightmare'])) $violations[] = 'auto_nightmare';
        if (!empty($env['auto_cdc']))       $violations[] = 'auto_cdc';
        if (!empty($env['auto_selenium']))  $violations[] = 'auto_selenium';
        if (!empty($env['auto_puppeteer'])) $violations[] = 'auto_puppeteer';
        if (!empty($env['proto_tampered'])) $violations[] = 'proto_tampered';
        if (!empty($env['perm_inconsistency'])) $violations[] = 'perm_inconsistency';
        if (isset($env['win_w']) && $env['win_w'] <= 0) $violations[] = 'win_w<=0';
        if (isset($env['scr_w']) && $env['scr_w'] <= 0) $violations[] = 'scr_w<=0';
        if (isset($env['scr_ah']) && isset($env['scr_h']) && $env['scr_ah'] > $env['scr_h']) $violations[] = 'scr_ah>scr_h';
        if (isset($env['color_depth']) && !in_array($env['color_depth'], [24, 32, 48])) $violations[] = 'color_depth=' . $env['color_depth'];
        if (isset($env['cpu_cores']) && $env['cpu_cores'] <= 0) $violations[] = 'cpu_cores<=0';
        if (isset($env['canvas_hash']) && (int)$env['canvas_hash'] === 0) $violations[] = 'canvas_hash=0';
        if (isset($env['webgl_data']) && is_array($env['webgl_data']) && (empty($env['webgl_data']['vendor']) || empty($env['webgl_data']['renderer']))) $violations[] = 'webgl_data_missing';
        if (isset($env['ls']) && $env['ls'] === false) $violations[] = 'ls=false';
        if (isset($env['idb']) && $env['idb'] === false) $violations[] = 'idb=false';
        if (isset($env['logic_check']) && (int)$env['logic_check'] >= 2) $violations[] = 'logic_check>=' . $env['logic_check'];

        if ($violations) {
            error_log('[NovaCMS Captcha DEBUG] env violations (allowed): ' . implode(', ', $violations));
        }
        return true; // 调试版一律放行
    }
